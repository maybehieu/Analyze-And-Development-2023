...
//		System.out.println(username+password);
	...
	public void addBookItem(HttpServletRequest request, HttpServletResponse response)
		...
		if (requireLogin && customerID <= 0) {
			...
			PrintWriter writer = response.getWriter();
			writer.write("Added " + bookItem.getBook().getTitle() + " to cart");
			writer.close();
		}
...
//		}
		...
		for (Map.Entry<Integer, Integer> entryBook : books.entrySet()) {
			...
			bookitemDAO.editBookItem(bookItem.getID(), bookItem.getPrice(), bookItem.getDiscount(), bookItem.getInStock()-entryBook.getValue());
			bookitems.add(bookItem);
...
//		response.sendRedirect("admin-index.jsp");  
	...
	public void editBookC(HttpServletRequest request, HttpServletResponse response)
		...
		int employeeID = 1;
		int ISBN = Integer.parseInt(request.getParameter("ISBN"));
		Book booktmp = bookDAO.getBookByID(ISBN);
		int authorID = booktmp.getAuthor().getID();
		int publisherID = booktmp.getPublisher().getID();
		Employee employee = employeeDAO.getEmployee(employeeID);
		String nameAuthor = request.getParameter("nameAuthor");
		String shortBio = request.getParameter("shortBio");
		String birth = request.getParameter("birth");
		Author authortmp = new Author(authorID, nameAuthor, shortBio, birth);
		bookDAO.editAuthor(authortmp);
		System.out.println(authortmp.getName());
//		Author author = bookDAO.getAuthor(bookDAO.getMaxIDAut());
		String address = request.getParameter("address");
		String namePublisher = request.getParameter("namePublisher");
		int operateYears = Integer.parseInt(request.getParameter("operateYears"));
		Publisher publishertmp = new Publisher(publisherID, address, namePublisher, operateYears);
		bookDAO.editPublisher(publishertmp);
//		Publisher publisher = bookDAO.getPublisher(bookDAO.getMaxIDPub());
		String title = request.getParameter("title");
		String summary = request.getParameter("summary");
		int years = Integer.parseInt(request.getParameter("years"));
		...
		if(type.equals("LightNovel")){
			...
			LightNovel book = new LightNovel(ISBN, title, summary, years, employee, authortmp, publishertmp, ID, translateLanguage, volume, editions);
...
//			writer.close();
			...
			String nameSeries = request.getParameter("nameSeries");
			String artists = request.getParameter("artists");
			Comics book = new Comics(ISBN, title, summary, years, employee, authortmp, publishertmp, ID, nameSeries, artists);
...
//			writer.close();
			...
			int volume = Integer.parseInt(request.getParameter("volumeTX"));
			int editions = Integer.parseInt(request.getParameter("editionsTX"));
			TextBook book = new TextBook(ISBN, title, summary, years, employee, authortmp, publishertmp, ID, numberOfPages, volume, editions);
...
//		orders.get(0).getCart().getCustomer().getUser().getAccount().getUsername()
...
//		txt.getAuthor().get
	...
	public void delBitem(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		boolean requireLogin = true;
		int customerID = getcustomerID(request);
		if (requireLogin && customerID != 1) {
...
//		txt.getAuthor().get\
		...
		bookitemDAO.delBookitem(bookitemID);
		RequestDispatcher dispatcher = request.getRequestDispatcher("/manage");
		dispatcher.forward(request, response);
	}
...
//		bookitem.getBook().getT
	...
	public void search(HttpServletRequest request, HttpServletResponse response)
		...
		List<Book> books = bookDAO.searchBook(keywords);
		List<Bookitem> bookitems = new ArrayList<>();
		for(Book book:books ){
			bookitems.add(bookitemDAO.getBookitem(book.getISBN()));
		}
		request.setAttribute("bookitems", bookitems);
		request.setAttribute("customerID", customerID > 0);
		RequestDispatcher dispatcher = request.getRequestDispatcher("booklistsearch.jsp");
		dispatcher.forward(request, response);
	}
}
